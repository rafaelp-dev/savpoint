...
public class UserGameService {
    ...
    public MessageResponse addGameToProfile(Authentication authentication, UserGameRequest request) {
        ...
        GameEntity game = gameRepository.findById(request.gameId())
                .orElseThrow(() -> new NotFoundException("Jogo não encontrado no banco de dados local."));

        if (userGameRepository
                .findByProfile_UserProfileIdAndGame_GameId(userProfile.getUserProfileId(), game.getGameId())
                .isPresent()) {
